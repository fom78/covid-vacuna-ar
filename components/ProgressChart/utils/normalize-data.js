...
export default function normalizeChartData() {
  ...
  // extract content with jurisdiccionNombre
  // --> files state: [..., ['2021', '01', '12'], ...]
  const jurisdiccionNombreHashMap = new Map();
  ...
  for (const i of files) {
    ...
    jurisdiccionNombreHashMap.set(mapKey, JSON.parse(mapValue));
  }

  // totals dataset
  const dataset = {};

  for (const [k, v] of jurisdiccionNombreHashMap) {
    const TOTALS = "Totales";
    const filterOnlyTotals = (v) => v.jurisdiccionNombre === TOTALS;
